...
export class TaskGantt implements AfterViewInit {
  ...
  analysis = input<ProjectAnalysis | null | undefined>(null);
  ...
  openComments = output<Task>();
  openTaskView = output<Task>();
  ...
  private buildTimelineMonths(startMs: number, endMs: number): Array<TimelineMonth> {
    ...
    const monthNames = this.core.monthNames();
    let cursor = startMs;
    while (cursor < endMs) {
      const current = new Date(cursor);
      const monthName = monthNames[current.getMonth()] ?? `M${current.getMonth() + 1}`;
      const nextMonthStart = this.addMonths(cursor, 1);
      months.push({
        label: `${monthName} ${current.getFullYear()}`,
        start: cursor,
        end: Math.min(nextMonthStart, endMs),
      });
  ...
  private computeTaskEnd(task: Task): Date {
    ...
    const durationWeeks = Math.max(task.durationWeeks ?? 0, 0);
    return new Date(start.getTime() + durationWeeks * this.weekMs);
  }

  private toDate(date: { year: number; month: number; week: number }): Date {
